...
class CLIPEncoder(nn.Module):
    ...
    def preprocess(self, image: Image.Image) -> torch.Tensor:
        return self._preprocess(images=image, return_tensors="pt")["pixel_values"][0]  # type: ignore
...
class DINOEncoder(nn.Module):
    def __init__(self, model_name="facebook/dinov2-with-registers-base"):
        ...
        from transformers import AutoImageProcessor, AutoModel, AutoConfig, Dinov2Model, Dinov2WithRegistersModel
        ...
        try:
            ...
            dino_config = AutoConfig.from_pretrained(model_name)
            self.backbone = AutoModel.from_config(dino_config)
...
class PerceptionEncoder(nn.Module):
    ...
    def preprocess(self, image: Image.Image) -> torch.Tensor:
        res = self._preprocess(image)  # type: ignore
